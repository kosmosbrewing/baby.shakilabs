...
export const FIRST_MEETING_GUIDE: GuideData = {
  ...
  sections: [
    ...
    {
      h2: "사용기한 게이지는 만료일에 맞춰 가득 찹니다",
      body: `화면의 사용기한 게이지는 출생일부터 지난 날수를 표시하고, 상한을 ${FIRST_MEETING_VALID_YEARS}년을 고정 일수로 환산하는 대신 계산된 만료일까지의 실제 날수로 잡습니다. 그래서 사이에 2월 29일이 끼는 아이는 상한이 731일, 그렇지 않은 아이는 730일이 되고 게이지가 가득 차는 날과 만료일이 언제나 같은 날입니다. 다만 게이지는 남은 기간만 보여 줄 뿐 잔액은 모르므로, 실제 잔액은 국민행복카드 발급 카드사 앱이나 고객센터에서 확인해야 합니다. 기한이 지나면 잔액이 소멸하고 환급이나 연장이 되지 않기 때문에, 잔액이 남았다면 만료 전에 사용 계획을 세워 두는 것이 좋습니다.`,
    },
    {
