...
const safeMessages = computed(() =>
  props.messages.length > 0 ? props.messages : ["shakilabs.com/baby는 입력값을 서버에 저장하지 않아요."]
);

const currentMessage = computed(() => safeMessages.value[currentIndex.value % safeMessages.value.length]);
